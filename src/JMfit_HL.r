...
# (generalized) linear mixed-effect models and 
# a survival model (Cox PH or Weibull), 
# via h-likelihood method. 
# It also addresses left-truncation of a continuous 
# longitudinal data fitted by a LME model.
...
JMfit_HL <- function(
  ...
  while(likDiff > itertol & Diff>Ptol & m<iterMax){
   ...
   # print("estimate dispersion parameters --- done.")
    ...
    Diff <- mean(c(abs((fixedest-fixedest0)/(fixedest0+10^(-6)))))
